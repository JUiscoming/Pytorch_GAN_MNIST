...
class GAN(object):
    def __init__(self, **kwargs):
        ...
        self.loaded_epoch = 0
        self.model_name = 'GAN'
        self.save_dir = ['models', 'GAN_MNIST']
        self.sample_dir = os.path.join(*['samples', 'GAN_MNIST'])
        ...
        self.G = Generator(data_dim = self.data_dim, input_dim = self.input_dim).to(device = self.device)
        self.D = Discriminator(data_dim = self.data_dim).to(device = self.device)
#        self.G_optimizer = optim.RMSprop(self.G.parameters(), lr = kwargs['lrG'])
#        self.D_optimizer = optim.RMSprop(self.D.parameters(), lr = kwargs['lrD'])
